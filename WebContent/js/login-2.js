...
var JLogin = React.createClass({
	...
	render : function() {
		...
		if(this.state.route == 'reg'){
			return (
					JTitleDiv({title:'Регистрация пользователя', key:this.state.key}, 
						React.DOM.div(null, 
							JInputText({ref: 'email', name:'Введите E-mail', width:'70pt', defaultValue:login, onKeyPress:this.onEnterKey}),
							JHref({style:{display:'inline-block', marginLeft: '70pt'}, onClick : this.onBack, caption:'Назад'}),
						...
						React.DOM.div(null, React.DOM.div({style:{width:'70pt', display:'inline-block'}}),
								
								JButton({style:{display:'inline-block'}, onClick : this.reguser, caption:'Зарегистрировать'})
						)
					)
			)
		}
		...
		if(this.state.route == 'forgot'){
			return (
					JTitleDiv({title:'Восстановление пароля'}, 
						React.DOM.div({key:this.state.key}, 
							JInputText({ref: 'email', name:'Введите Ваш логин или E-mail', width:'70pt', defaultValue:login, onKeyPress:this.onEnterKey}),
							JHref({style:{display:'inline-block', marginLeft: '70pt'}, onClick : this.onBack, caption:'Назад'}),
						...
						React.DOM.div({className: 'g-recaptcha', id:'rdiv'}),
						React.DOM.div(null, React.DOM.div({style:{width:'70pt', display:'inline-block'}}),								
								(this.state.forgotOk)?null:JButton({style:{display:'inline-block'}, onClick : this.resuser, caption:'Восстановить'})
						)
					)
			)
		}
		return (
				React.DOM.div({key:this.state.key}, 
					React.DOM.div(null,
						JInputText({ref: 'name', name:'Логин', width:'70pt', autocomplete:'true', defaultValue:login, onKeyPress:this.onEnterKey, placeholder:'Введите логин'}),
						JInputText({ref: 'password', name:'Пароль', width:'70pt', onKeyPress:this.onEnterKey, type: 'password', placeholder:'Введите пароль'}),
					...
					React.DOM.div(null, 
							JButton({className: 'ui-btn ui-shadow ui-corner-all ui-btn-inline div-100', style:{display:'inline-block', width:'120pt', marginLeft: '0pt'}, onClick : this.login, caption:'Вход'})
					),
					React.DOM.div(null, 
							JHref({style:{display:'inline-block', margin:'.1em .5em', width:'120pt', marginLeft: '70pt'}, onClick : this.onRegister, caption:'Регистрация'})
					),
					React.DOM.div(null, 
							JHref({style:{display:'inline-block', margin:'.1em .5em', width:'120pt', marginLeft: '70pt'}, onClick : this.onForgot, caption:'Забыл пароль?'})
					)
				)
